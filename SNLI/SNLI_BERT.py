...
from SNLIdata.SNLIconfig import filenames, parameters
...
def prepareBatches(ids, id2content):
    ...
    for head_id in range(0, len(ids), parameters.batch_size):
        ...
        for _id in batch_ids:
            ...
            #print(id2content[_id]["en_defs"])
            #print("pre",len(id2content[_id]["en_defs"][0]),len(id2content[_id]["en_defs"][1]))
        ...
        for i in range(len(batch["inputs"])):
            ...
            #print(len(batch["inputs"][i]), torch.LongTensor(batch['inputs'][i]).shape)
...
if __name__=="__main__":
    ...
    label2id = json.load(open("SNLIdata/" + filenames.label2id, 'r', encoding='utf-8'))
    ...
    train_ids = json.load(open("SNLIdata/"+filenames.train_set, 'r', encoding='utf-8'))
    valid_ids = json.load(open("SNLIdata/"+filenames.valid_set, 'r', encoding='utf-8'))
    test_ids = json.load(open("SNLIdata/"+filenames.test_set, 'r', encoding='utf-8'))
    id2content = json.load(open("SNLIdata/"+filenames.id2content, 'r', encoding='utf-8'))
