...
"""
class BILSTM_sememe(nn.Module):
    def __init__(self, config):
        super(BILSTM_sememe, self).__init__()
    ...
    def forward(self, inputs):
        ...
        sent_output = torch.cat([a, b], dim = 2)
        idx_unsort = torch.from_numpy(idx_unsort).cuda()
        sent_output = sent_output.index_select(1, idx_unsort)

        # Pooling
        if self.pool_type == "mean":
            sent_len = torch.FloatTensor(length.copy()).unsqueeze(1).cuda()
            emb = torch.sum(sent_output, 0).squeeze(0)
            emb = emb / sent_len.expand_as(emb)
        elif self.pool_type == "max":
            if not self.max_pad:
...
class ClassificationNet(nn.Module):
    def __init__(self, config):
        ...
        self.inputdim = self.enc_lstm_dim if (self.encoder_type =="LSTMEncoder" or self.encoder_type == "LSTM_sememe") else self.inputdim
        self.classifier = nn.Sequential(
